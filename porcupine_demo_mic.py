...
def news():
    ...
    speak_text_cmd('Sir, today news headlines are:')
    if not allheadlines:
        speak_text_cmd('Sorry, sir, unable to fetch news. As the internet and I are not connected.')
    for hl in allheadlines[:10]:  # Print only the top 10 headlines
        index_number = index_number + 1
        hll = str(hl).replace(' -', ', from')
        speak_text_cmd('Number ' + str(index_number) + '. ' + hll)
...
def play_sound(file_path):
    ...
    try:
        ...
        sound.play()
        pygame.time.delay(int(sound.get_length() * 900))  # Delay to ensure the sound is played completely
...
def read_voice_cmd():
    ...
    if not voice_text:
        ...
        if audcheck == 2:
            ...
            play_sound("C:\\users\\mashu\\stop.mp3")
...
def reader():
    ...
    speak_text_cmd('The selected text is -' + data)
    speak_text_cmd("That's All")
...
def weather(cmd):
    try:
        ...
        if x["cod"] != "404":
            ...
            speak_text_cmd("sorry  your current location is not found")
    except ConnectionError:
        speak_text_cmd('sorry  internet and I not working')
    except :
        speak_text_cmd('sorry , unable to fetch weather information as Internet and I is not connected.')
        
def shutdown_sys(voice_note):
    password = read_voice_cmd().lower()
    if '8' in password and '0' in password and '2' in password and '5' in password:
        if 'shutdown' in voice_note or 'shut' in voice_note:
            speak_text_cmd('shutting down system')
            speak_text_cmd(' Bye  and take care ')
            os.system('shutdown -s')
        else:
            speak_text_cmd("Restarting The system")
            os.system('shutdown -r')
        exit()
    else:
        if 'cancel' in password or 'shut' in password:
            speak_text_cmd('As you wish ')
            ...
            speak_text_cmd('sorry  password doesnot match')
...
def main():
    ...
    if failsafe_check == 0:
            ...
            if hour >= 5 and hour < 12:
                ...
                speak_text_cmd('I am ready, Just call me,when you needed ?')
    else :
        speak_text_cmd('After solving the problem, please let me know')
    print(':: waiting for wakeword detection ::')
    ...
    try:
        while True:
            ...
            if result >= 0:
                ...
                audcheck = 0
                if failsafe_check > 0 :
                    speak_text_cmd('Yes, did the glitch of my program is solved:')
                    voice_note = read_voice_cmd().lower()
                    if 'yes' in voice_note or 'solve' in voice_note:
                        speak_text_cmd('Thank you for fixing my problem')
                        ...
                        with open(__file__, 'w') as f:
                            f.write('\n'.join([new_line] + lines[1:]))
                        speak_text_cmd('Now please wait a second. let me update my program to continue')
                        ...
                        speak_text_cmd('Its okay now tell me  what can i do for you ?')
                else:
                    # play_sound("C:\\users\\mashu\\start.mp3")      
                    speak_text_cmd('Yes')        
                    while audcheck < 2:
                        try:
                            ...
                            vnote = voice_note
                            if voice_note == 'miami':
                                speak_text_cmd('yes , what can i do for you ?')
                                continue
                            if voice_note == 'open' :
                                speak_text_cmd('what do you want me to open  ?')
                                newcm = read_voice_cmd().lower()
                            ...
                            #     continue
                                ...
                                speak_text_cmd(random.choice(responses))                           
                                ...
                                speak_text_cmd('because you have created me')
                                continue
                                ...
                                if 'you know me' in voice_note:
                                    speak_text_cmd('Yes, i know You , you are my Creater, Mr.Mashud')
                                    continue
                                else:
                                    speak_text_cmd("Mashud")
                                    continue
                            elif 'open facebook' in voice_note:
                                webbrowser.open("https://www.facebook.com/")
                                speak_text_cmd("opening ")
                                continue
                                ...
                                speak_text_cmd('opening your mails ')
                                continue
                                ...
                                if log_info == "unlocked" :
                                    speak_text_cmd(" your pc is already unlocked")
                                else:
                                    '''
                                ctypes.windll.user32.LockWorkStation()
                                speak_text_cmd("unlocking :")
                                pyautogui.press('enter')
                                pyautogui.press('enter')
                                speak_text_cmd('Please use password to unlock  ')
                                continue
                                ...
                                if 'up' in voice_note:
                                    volume_up()
                                    speak_text_cmd('Volumed up ')
                                    continue
                                elif 'down' in voice_note:
                                    volume_down()
                                    speak_text_cmd('Volumed Downed ')
                                    continue
                                elif 'set' in voice_note or 'by' in voice_note:
                                    percentage = re.findall(r'[0-9]+', voice_note)
                                    if len(percentage) == 0:
                                        speak_text_cmd("By How many percent ?")
                                        voice_note = read_voice_cmd().lower()
                                        percentage = re.findall(r'[0-9]+', voice_note)
                                        if len(percentage) == 0:
                                            speak_text_cmd("Sorry, I am not able to understand")
                                            pass
                                        else:
                                            per = int(percentage[0])
                                            set_volume(per)
                                            speak_text_cmd('volume set to ' + str(per) + ' percent ')
                                            continue
                                    else:
                                        per = int(percentage[0])
                                        set_volume(per)
                                ...
                                if 'google' in voice_note:
                                    ...
                                    speak_text_cmd('Searching on google ')
                                    continue
                                    ...
                                    speak_text_cmd('searching on youtube ')
                                    continue
                                ...
                                if 'shut' in voice_note:
                                    speak_text_cmd(" i need confirmation password to shutdown the system")
                                elif 'rest' in voice_note or 'boot' in voice_note:
                                    speak_text_cmd(' i need confirmation password to restart the system')
                                speak_text_cmd("please tell me the confirmation password ?")
                                shutdown_sys(voice_note)
                                ...
                                if 'open' in voice_note:
                                    ...
                                    speak_text_cmd('sorry i did not got it please try again')
                                    continue
                            elif ( 'notepad' in voice_note or 'write something' in voice_note) and 'i ' in voice_note or 'note something' in voice_note or 'notepad' in voice_note:
                                if 'open' in voice_note or 'something' in voice_note:
                                    ...
                                    if 'open' in voice_note:
                                        ...
                                        speak_text_cmd('ok  i am opening notepad for you')
                                        continue
                                elif 'close' in voice_note:
                                    ...
                                    speak_text_cmd('sorry  i didnot got it please try again')
                                    continue
                            elif 'window' in voice_note:
                                if 'close' in voice_note:
                                    pyautogui.hotkey('alt', 'f4')
                                    speak_text_cmd('closed ')
                                    continue
                                    ...
                                    speak_text_cmd('sorry  i did not got it, please say it once')
                            elif 'thank' in voice_note and 'you' in voice_note :
                                speak_text_cmd("You're welcome . i'm just doing my job.")
                                continue
                            elif 'weather' in voice_note or 'temperature' in voice_note:
                                if 'status' in voice_note or 'report' in voice_note :
                                    weather('status')
                                    continue
                                ...
                                speak_text_cmd('Fetching todays news please wait')
                                news()                            
                                ...
                                if('coding' in voice_note):
                                    speak_text_cmd('Extending display and opening vs code.')
                                else:
                                    speak_text_cmd("Extending display.") 
                                ...
                                if('gaming' in voice_note):
                                    speak_text_cmd('Enjoy Gaming')
                                elif('dinner' in voice_note):
                                    speak_text_cmd("enjoy dinner time and what you want to prefer Youtube or Netflix?")
                                    preference = read_voice_cmd()
                            ...
                            #     alarm(voice_note)
                                ...
                                try:
                                    ...
                                    if "this pc" in voice_note:
                                        ...
                                        speak_text_cmd("Sorry  there is no drive like you said")
                                        continue
                                except IndexError:
                                    pass
                            elif ('right' in voice_note or 'write' in voice_note) and 'something' in voice_note:
                                speak_text_cmd('ok  go to space where you want to type')
                                speak_text_cmd('Now tell me  what to right')
                                voice_note = read_voice_cmd().lower()
                                pyautogui.write(voice_note)
                                while True:
                                    speak_text_cmd('done , and do you want to right more ')
                                    voice_note = read_voice_cmd().lower()
                                    if 'yes' in voice_note:
                                        speak_text_cmd('ok  tell me what to right next')
                                        voice_note = read_voice_cmd().lower()
                                        pyautogui.write(' ' + voice_note)
                                        continue
                                    else:
                                ...
                                while True:
                                    speak_text_cmd('done , and do you want to write more ')
                                    voice_note = read_voice_cmd().lower()
                                    if 'yes' in voice_note:
                                        speak_text_cmd('ok  tell me what to write next')
                                        voice_note = read_voice_cmd().lower()
                                        pyautogui.write(' ' + voice_note)
                                        continue
                                    else:
                                ...
                                strTime = datetime.datetime.now().strftime("%H,%M")
                                if hour >= 0 and hour < 12:
                                    speak_text_cmd(f", the time is {strTime},AM")
                                else:
                                    lem = int(strTime[:2])
                                    lem = (lem - 12)
                                    strTime = datetime.datetime.now().strftime('%M')
                                    speak_text_cmd(f", the time is {lem, strTime},PM")
                                continue
                            elif 'read' in voice_note:
                                speak_text_cmd('ok  make sure you have already selected what to read')
                                reader()
                            elif 'speed' in voice_note and ('net' in voice_note or 'network' in voice_note):
                                speak_text_cmd('showing the result in new window ')
                                os.system("start /B start cmd.exe @cmd /k speedtest-cli ")
                                continue
                            elif ('like' in voice_note or 'love' in voice_note) and 'me' in voice_note:
                                speak_text_cmd("Yes! A thousand times yes")            
                            elif 'do ' in voice_note or 'what' in voice_note or (
                                    'who' in voice_note or 'where' in voice_note and 'is' in voice_note):
                                speak_text_cmd(
                                    'Sorry  i am facing a glitch in my program to answer Your question. but i am showing you the Answer with the help of google. Hope you will get satisfy by the answer')
                                webbrowser.open('https://www.google.com/search?q={}'.format(voice_note))
                                time.sleep(2)
                                ...
                                speak_text_cmd(' do you want me to open {}, application ?'.format(voice_note))
                                confirmation = read_voice_cmd().lower()
                                ...
                                speak_text_cmd("Playing on youtube")
                                play_first_youtube_video(voice_note)
                                continue
                            elif any(keyword in voice_note for keyword in ['sleep', 'stop ask', 'stop lis']):
                                speak_text_cmd('Ok, call me when you need assistance.')
                                audcheck = 2
                                print(':: Waiting for wakeword detection ::')
                                slp = 1
                            elif re.search(r'\b(bye|goodbye)\b', voice_note, re.IGNORECASE):
                                # Your code for handling goodbye actions
                                speak_text_cmd(random.choice(["Goodbye!", "See you later!", "Take care!"])+" and don't forget to call me when you needed")
                                audcheck = 2
                                print(':: Waiting for wakeword detection ::')  
                                slp = 1 
                                print(':: Waiting for wakeword detection ::')                    
                                ...
                                if 'boot' in voice_note or 'start' in voice_note:
                                    speak_text_cmd('okay I am Rebooting my Program. please wait, i will be online soon')
                                else:
                                    speak_text_cmd('okay I am updating my Program. please wait, i will be online soon')
                                os.system('porcupine_demo_mic --access_key uX5rsH7XMmSI5Aey1irLgr+R4OiwiTZch0L/JIIZLwZDjCZgauAaZg== --keywords miami --audio_device_index 1')
                                exit()
                            elif 'goodnight' in voice_note or 'night' in voice_note:
                                speak_text_cmd(random.choice(["Goodnight!", "Sweet dreams!", "Sleep well!"])+" and do you want me to shut down the system?")                            
                                altq = read_voice_cmd().lower()
                            ...
                            if e == 'listening timed out while waiting for phrase to start':
                                ...
                                with open(__file__, 'w') as f:
                                    f.write('\n'.join([new_line] + lines[1:]))
                                speak_text_cmd('There is a glitch in my program. please solve it, i am opening the source code and error details for you.')
                                m = 'C:\\Users\\mashu\\AppData\\Local\\Programs\\Python\\Python312\\Lib\\site-packages\\pvporcupinedemo\\porcupine_demo_mic.py'
                                os.system("start {}".format(m))
